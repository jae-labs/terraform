...
This repository is the source of truth for all `jae-labs` infrastructure as code. It manages GitHub organization config, Cloudflare DNS and Pages, Doppler secrets infrastructure, Honeycomb datasets, OCI infrastructure, Sentry organization/project config, and Tailscale preferences/ACLs. Changes are split by provider root, validated in CI, and auto-applied on merge to `main`.
...
- All domains (`github/`, `cloudflare/`, `doppler/`, `honeycomb/`, `oci/`, `sentry/`, and `tailscale/`) are flat and self-contained Terraform roots.
...
Swap `github` for `cloudflare`, `doppler`, `honeycomb`, `oci`, `tailscale`, or `sentry` as needed.
...
for root in github cloudflare doppler honeycomb oci tailscale sentry; do
  (
    cd "${root}" && \
...
See [CONTRIBUTING.md](https://github.com/jae-labs/terraform?tab=contributing-ov-file).
...
[![Star History Chart](https://api.star-history.com/svg?repos=jae-labs/terraform&type=date&legend=top-left)](https://www.star-history.com/#jae-labs/terraform&type=date&legend=top-left)
