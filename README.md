...
This repository is the source of truth for all `jae-labs` infrastructure as code. It manages GitHub organization config, Cloudflare DNS and Pages, Doppler secrets infrastructure, and OCI infrastructure. Changes are split by provider root, validated in CI, and auto-applied on merge to `main`.
...
- All domains (`github/`, `cloudflare/`, `doppler/`, and `oci/`) are flat and self-contained Terraform roots.
...
for root in github cloudflare doppler oci; do
  (
    cd "${root}" && \
...
`Contract` means the bot depends on `concierge-schema.yaml` matching the editable locals data it exposes. The schema and those locals files are not internal implementation details. They are an interface consumed by another system.
