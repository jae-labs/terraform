...
resource "cloudflare_account_member" "members" {
  ...
  status     = try(each.value.status, "accepted")
}
...
resource "cloudflare_ruleset" "custom_waf" {
  ...
  name    = try(each.value.waf_ruleset_name, "Zone-level custom rules")
  kind    = try(each.value.waf_ruleset_kind, "zone")
  phase   = try(each.value.waf_ruleset_phase, "http_request_firewall_custom")
...
#
# Purpose:
#   Create Cloudflare Zero Trust tunnels for secure outbound-only connections
#   from origin infrastructure to the Cloudflare edge network.
#
# How it works:
#   1. Iterates over each tunnel defined in local.tunnels.
#   2. Creates the tunnel with config_src = "cloudflare", enabling remote
#      configuration of ingress rules via Terraform.
#   3. The cloudflared run token is retrieved separately from Cloudflare and
#      distributed to origin hosts outside Terraform.
# ============================================================================
resource "cloudflare_zero_trust_tunnel_cloudflared" "tunnels" {
...
#
# Purpose:
#   Configure ingress rules for each Cloudflare Zero Trust tunnel, mapping
#   public hostnames to local services running on the origin.
#
# How it works:
#   1. Iterates over each tunnel defined in local.tunnels.
#   2. Routes requests for the configured hostname to the specified local
#      service (e.g., an HTTPS server on the origin).
#   3. Disables origin TLS verification for self-signed localhost
#      certificates on the origin host.
#   4. A catch-all rule returns HTTP 404 for any unmatched hostname.
# ============================================================================
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "tunnels" {
  for_each = local.tunnels
    ...
    ingress = [
      {
        ...
        service  = each.value.local_service
        origin_request = {
          no_tls_verify = try(each.value.no_tls_verify, true)
        }
      },
      {
        service = try(each.value.catch_all_service, "http_status:404")
      }
    ]
  }
}
