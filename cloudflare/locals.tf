locals {
  ...
  dns_records = {
    "justanother.engineer" = {
      ...
      "oci-tunnel" = {
        type    = "CNAME"
        name    = "cf-oci"
        content = "${cloudflare_zero_trust_tunnel_cloudflared.tunnels["oci"].id}.cfargotunnel.com"
        proxied = true
        comment = "OCI instance via Cloudflare Tunnel"
      }
      "root-cname" = {
        type    = "CNAME"
  ...
  #
  # How it works:
  #   1. Defines each tunnel by a unique key (e.g., "oci").
  ...
  tunnels = {
    "oci" = {
      name          = "oci-tunnel"
      hostname      = "cf-oci.justanother.engineer"
